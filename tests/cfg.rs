...
#[test]
fn ignore_dependency_from_other_platform() {
    let this_family = if cfg!(unix) {"unix"} else {"windows"};
    let other_family = if cfg!(unix) {"windows"} else {"unix"};
    Package::new("foo", "0.1.0")
        .optional_dep("bar", "0.2.0")
        .feature("feat1", &["bar"])
        .publish();
    ...
    let p = project("a")
        .file("Cargo.toml", &format!(r#"
            ...
            [target.'cfg({})'.dependencies]
            foo = {{ version = "0.1.0", default-features = true }}

            [target.'cfg({})'.dependencies]
            foo = {{ version = "0.1.0", default-features = false, features = ["feat1"] }}
        "#, this_family, other_family))
        .file("src/lib.rs", "extern crate foo;");
